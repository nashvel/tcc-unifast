  ...
  <div class="min-h-screen bg-bg">
      ...
      <div class="flex h-14 shrink-0 items-center gap-2 border-b px-4">
        ...
        <div class="min-w-0 leading-tight">
          <p class="truncate text-sm font-semibold text-sidebar-text">
            {{ isStudent ? "Student Portal" : "UniFAST TES" }}
          </p>
          <p class="truncate text-2xs text-sidebar-text-muted">
            {{ isStudent ? "UniFAST TES" : "Grantee Management" }}
          </p>
        </div>
      </div>
    ...
    <div class="min-h-screen lg:pl-60">
      <header class="sticky top-0 z-30 flex h-14 items-center gap-2 border-b bg-surface px-3">
        <button class="rounded-md p-1.5 hover:bg-surface-muted lg:hidden" @click="mobile = true">
          <IconMenu2 :size="18" />
        </button>
        ...
        >
          <IconSearch :size="15" /><span class="flex-1 truncate text-sm">Search or jump to…</span
          ><kbd
        ...
        <div class="relative">
          ...
          >
            <div class="flex h-10 items-center justify-between border-b px-3">
              ...
              <button class="text-xs text-primary">Mark all read</button>
            </div>
            <div class="space-y-3 p-3 text-xs">
              <p>
                <b>Documents validated</b><br /><span class="text-text-muted"
                  >12 records are ready for eligibility review.</span
                >
              </p>
              <p>
                <b>Batch closing soon</b><br /><span class="text-text-muted"
                  >TES Semester 2 closes in five days.</span
                >
              </p>
            </div>
          </div>
        ...
        <div class="relative">
          ...
          >
            <DiceBearAvatar
              ...
              :alt="isStudent ? 'Maria Santos' : 'System Administrator'"
              :size="28"
            />
            <span class="hidden text-left leading-tight sm:block"
              ><span class="block text-xs font-medium">{{
                user?.name ?? (isStudent ? "Maria Santos" : "System Administrator")
              }}</span
          ...
          >
            ...
            >
              <IconUserCircle :size="15" /> Profile</button
            ><button

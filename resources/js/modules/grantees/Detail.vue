...
const detailQuery = useQuery({
  queryKey: computed(() => queryKeys.grantee(String(route.params.id))),
  queryFn: async () => {
    const payload = await apiFetch<{ data: GranteeDetail }>(`/api/grantees/${route.params.id}`);
    return payload.data;
  },
});

const grantee = computed(() => detailQuery.data.value);

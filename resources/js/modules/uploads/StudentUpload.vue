...
async function submit() {
  ...
  try {
    const response = await fetch("/api/student/submissions/ocr", {
      method: "POST",
    ...
    if (!response.ok) {
      throw new Error(payload.errors?.file?.[0] || payload.message || "OCR processing failed.");
    }
    ...
    result.value = {
      ...
      confidence: ocr.document_type === "image" ? ocr.result.average_confidence : null,
    };
  } catch (exception) {
    error.value =

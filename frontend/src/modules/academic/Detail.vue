...
import { useAcademicDetail } from "@/composables/useAcademic";
import type { CourseRemark } from "@/api";

const route = useRoute();
const { record, query: recordQuery } = useAcademicDetail(String(route.params.id));
...
<template>
  <div>
    ...
    <div v-if="recordQuery.isLoading.value" class="space-y-4">
      <StatGridSkeleton :count="5" />
      <CardSkeleton :lines="6" />
    </div>
    <EmptyState
      v-else-if="recordQuery.isError.value"
      variant="error"
      title="Couldn't load academic record"
      :hint="
        recordQuery.error.value instanceof Error
          ? recordQuery.error.value.message

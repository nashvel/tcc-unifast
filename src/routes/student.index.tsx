...
export const Route = createFileRoute("/student/")({
  component: () => {
    ...
    return (
      <div>
        <PageHeader title="Welcome, Maria" description="Here's your TES application overview." />
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 mb-4">
          <StatCard label="Account status" value="Active" icon={IconUserCheck} tone="success" />
          <StatCard label="Profile completion" value="85%" icon={IconCircleCheck} tone="primary" hint={
            <div className="h-1 mt-1 rounded-full bg-surface-muted overflow-hidden"><div className="h-full bg-primary" style={{ width: "85%" }} /></div>
          } />
          <StatCard label="Documents submitted" value={`${submitted} / ${requiredDocs.length}`} icon={IconFileCheck} tone="info" />

...
function DocDetail() {
  ...
  const doc = useDocumentStore((s) => s.docs.find((d) => d.id === id));
  const update = useDocumentStore((s) => s.updateStatus);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        <div className="lg:col-span-2 space-y-4">
          <Panel title="File Preview" icon={IconFile}>
            ...
            <p className="text-xs text-text-muted mt-2">Uploaded {doc.uploadedAt}</p>
          </Panel>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <Panel title="Extracted vs On-record">
              <table className="w-full text-xs">
                ...
                <tbody>
                  <tr className="border-t"><td className="py-1.5">Name</td><td>{doc.ocr?.name ?? "—"}</td><td>{doc.granteeName}</td></tr>
                  <tr className="border-t"><td className="py-1.5">Student #</td><td>{doc.ocr?.studentNo ?? "—"}</td><td>{doc.studentNumber}</td></tr>
                </tbody>
              </table>
            </Panel>
            <Panel title="Face Verification / Liveness" icon={IconCameraSelfie}>
              <div className="flex items-center gap-3">
                ...
                <div className="text-xs">
                  <p>Match score: <span className="font-semibold">{100 - doc.riskScore}%</span></p>
                  <p className="text-text-muted">Liveness: {doc.riskScore < 50 ? "Pass" : "Review"}</p>
                </div>
              </div>
            </Panel>
          </div>
        ...
        <div className="space-y-4">
          ...
          <Panel title="Staff Decision">
            ...
            <div className="grid grid-cols-2 gap-2 mt-3">
              <Btn variant="primary" icon={IconCheck} onClick={() => act("approved")}>Approve</Btn>
              <Btn variant="danger" icon={IconX} onClick={() => act("rejected")}>Reject</Btn>
              <Btn variant="outline" icon={IconRefresh} onClick={() => act("resubmission")}>Resubmit</Btn>
              <Btn variant="outline" icon={IconShieldExclamation} onClick={() => act("suspicious")}>Suspicious</Btn>
            </div>
          </Panel>
        </div>
      </div>

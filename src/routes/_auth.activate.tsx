...
function ActivatePage() {
  ...
  const activate = useMasterlistStore((s) => s.activateAccount);
  const [step, setStep] = useState<Step>("verify");
  const [studentNumber, setStudentNumber] = useState("2024-00124");
  const [birthdate, setBirthdate] = useState("2004-02-20");
  const [contact, setContact] = useState("jm.santos@up.edu.ph");
  ...
  function verify(e: React.FormEvent) {
    ...
    const match = rows.find(
      (r) =>
        r.studentNumber === studentNumber.trim() &&
    ...
    if (match.accountStatus === "active") {
      setError("This account is already active. Please sign in instead.");
      return;
    }
    if (match.accountStatus === "duplicate" || match.accountStatus === "invalid") {
      ...
      {step === "verify" ? (
        <form onSubmit={verify} className="mt-5 space-y-4">
          <FormField label="Student Number" required helper="As listed on your school record.">
            <TextInput value={studentNumber} onChange={(e) => setStudentNumber(e.target.value)} placeholder="2024-00123" />
          </FormField>
